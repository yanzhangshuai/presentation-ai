...
const examples = ref<ExamplePrompt[]>(EXAMPLE_PROMPTS.slice(0, 6))

const onShuffle = () => {
  examples.value = shuffle(EXAMPLE_PROMPTS).slice(0, 6)
}
</script>
    ...
    <div class="grid gap-4 grid-cols-1 sm:grid-cols-3">
      ...
      >
        <div
          ...
          :style="{ background: e.color.background, color: e.color.color }"
        >
          <span class="text-lg">{{ e.icon }}</span>
        </div>

        <span class="line-clamp-2 flex-1 text-sm font-medium text-card-foreground">
          {{ e.title }}
        </span>
      </UButton>
    </div>
